...
void Init() {
  ...
  // SPI.setClockDivider(SPI_CLOCK_DIV2); //Sts SPI clock to 4 MHz for an 8 MHz system clock
	...
	RTC.ClearAlarm(); //
	Downhole.begin(TP2v2);
  #if defined(BARO) 
  	bmp.begin(0x76, 0x58); //Initialize BMP with alternate address
  #endif
}
...
void GetTPDownholeVals() {
  ...
  //Get pressure, get temp1, get temp2
	Pressure[0] = Downhole.getPressure();
// Serial.println(Pressure[0]); //DEBUG!
	Temp[0] = Downhole.getTemperature(0);
	// float Vcc = 3.3;
	// float Val = Vcc - adc.GetVoltage();
 //  	float TempData = TempConvert(Val, Vcc, 47000.0, A, B, C, D, 10000.0);
  	Temp[1] = Downhole.getTemperature(1); //Get temp from on board thermistor 
}

void GetBaroVals() {
  //Get values from barometer
  //Get pressure, and temp
  	Serial.println(bmp.readTemperature()); //DEBUG!
  	Pressure[1] = bmp.readPressure()*0.01; //Read pressure and convert from pascals to mBars
	Temp[4] = bmp.readTemperature();
}
